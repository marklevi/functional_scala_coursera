...
class FunSetSuite extends FunSuite {
  ...
  test("rest of the problems") {
    new TestSets {
      def a(x: Int) = x < 20

      def b(x: Int) = x > 10

      val s: Set = intersect(a, b)

      assert(contains(s, 15), "Intersect 15")
      assert(!contains(s, 3), "Intersect 3")

      assert(contains(diff(a, b), 5))
      assert(!contains(diff(a, b), 15))

      assert(contains(filter(a, b), 15))
    }
  }
  ...
  test("forall exists map") {
    new TestSets {
      ...
      assert(exists(a, x => x * x < 60), "exists")
